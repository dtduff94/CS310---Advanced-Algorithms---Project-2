...
public class DegreesOfSeparationBFS {
    ...
    public static void printDegreesOfSeparation(SymbolGraph sg, BreadthFirstPaths bfs, String sink)
    {
        ...
        // the bacon number in addition to the path.
         if (sg.contains(sink)) {
                ...
                if (bfs.hasPathTo(t)) {
                    ...
                    // Now print the path. You have to print it in reverse order to what's in the
                    ...
                    while (path.size() > 1) {
                        ...
                        int vnext = path.peek();
                        if (actor) {
                            StdOut.printf("%s was in the movie %s with %s\n", sg.nameOf(vac), sg.nameOf(vm), sg.nameOf(vnext));
                        }
                    }
                }
              else {
	...
	String actor     = args[3];
        ...
        printDegreesOfSeparation(sg, bfs, actor);
    }
}
